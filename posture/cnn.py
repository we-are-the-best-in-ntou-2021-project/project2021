...
data_arm = data[:,:,1:8,:]
data_leg = data[:,:,8:15,:]
data = np.concatenate((data_arm,data_leg), axis=-2)
"""
...
"""
data_test_arm = data_test[:,:,1:8,:]
data_test_leg = data_test[:,:,8:15,:]
data_test = np.concatenate((data_test_arm,data_test_leg), axis=-2)
"""
...
if os.path.exists("./model_weight/model.h5"):
  ...
  model.add(Conv1D(16,kernel_size=10,padding='same',activation="relu"))
