# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 08:16:08 2021
...
data_arm = data[:,:,1:8,:]
data_arm = data_arm.reshape((data_arm.shape[0],data_arm.shape[1],-1))
data_leg = data[:,:,8:15,:]
data_leg = data_leg.reshape((data_leg.shape[0],data_leg.shape[1],-1))
...
data_test_arm = data[:,:,1:8,:]
data_test_arm = data_test_arm.reshape((data_test_arm.shape[0],data_test_arm.shape[1],-1))
data_test_leg = data[:,:,8:15,:]
data_test_leg = data_test_leg.reshape((data_test_leg.shape[0],data_test_leg.shape[1],-1))
"""
...
data = data[index]
labels = labels[index]
# data_test, labels_test = shuffle(data_test, labels_test, random_state = 0)a
...
if os.path.exists("./model_weight/model.h5"):
  ...
  input_x = Input(shape=(data_arm.shape[1:]))
  input_y = Input(shape=(data_test_arm.shape[1:]))
  
  cnn1_x = Conv1D(32, kernel_size=10, padding='valid',activation='relu')(input_x)
  maxpool1_x = MaxPooling1D(pool_size=2)(cnn1_x)
  batch1_x = BatchNormalization()(maxpool1_x)
  """
  ...
  """
  cnn1_y = Conv1D(32, kernel_size=10, padding='valid',activation='relu')(input_y)
  maxpool1_y = MaxPooling1D(pool_size=2)(cnn1_y)
  batch1_y = BatchNormalization()(maxpool1_y)
  """
  ...
  merge = concatenate([batch1_x,batch1_y])
  ...
  """
  cnn = Conv1D(64, kernel_size=3, padding='same', activation='relu')(merge)
  ...
  train_history = model.fit([data_arm,data_leg],labels,batch_size=16,epochs=300,verbose=1,validation_split=0.2)
  #model.save("model_weight/model.h5")
  
  score = model.evaluate([data_test_arm, data_test_leg],labels_test,verbose=0)
  test_loss =score[0]
  
  predictions = model.predict([data_test_arm,data_test_leg])
  test_accuracy = sum([np.argmax(labels_test[i])==np.argmax(predictions[i]) for i in range(len(labels_test))])/len(labels_test)

  predictions = model.predict_classes([data_test_arm,data_test_leg])
