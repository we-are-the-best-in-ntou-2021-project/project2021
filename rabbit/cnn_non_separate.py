...
if not os.path.exists("./model_weight/model.h5"):
  ...
  if os.path.exists("json_data_non_seperate/data.npy"):
    data = load('json_data_non_seperate/data.npy')
    data_test = load('json_data_non_seperate/data_test.npy')
    labels = load('json_data_non_seperate/labels.npy')
    labels_test = load('json_data_non_seperate/labels_test.npy')
  else:
    a = de.JasonDecoder(dataset_name=action_name,frame=frame,dirname=dirname,shift=shift)
    data, labels = a.decoding()
    labels = np.array(labels)
    save('json_data_non_seperate/data.npy',data)
    save('json_data_non_seperate/labels.npy',labels)
    ...
    save('json_data_seperate/data_test.npy',data_test)
    save('json_data_seperate/labels_test.npy',labels_test)
  ...
  message = "Epochs = %s\nOffset: %s\nLoss : %s \nAccuracy : %s" % (epochs,offset,test_loss,test_accuracy)
  r = requests.post(
