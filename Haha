...
def make_action_data():
    X = np.load('./json_data/data.npy')
    X = X.reshape(X.shape[0], frames, nodes*channels)
    
    Y_action = np.load('./json_data/labels_action.npy')
    Y_action = to_categorical(Y_action)
    
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y_action, test_size = 0.1, random_state=0)
...
def make_ID_data():
    ...
    Y_action = np.load('./json_data/labels_ID.npy')
    Y_action = to_categorical(Y_action)
...
ID_classifier.fit(x = X_train_2, y = Y_train_2, validation_split = 0.2, epochs = 100, batch_size=50)
