...
a = de.JasonDecoder(dataset_name=action_name,frame=50,shift=5)
...
# data /= 3000
data,data_test,labels,labels_test = train_test_split(data,labels,test_size = 0.2,random_state = 20)
labels = keras.utils.to_categorical(labels)
labels_test = keras.utils.to_categorical(labels_test)
#print(labels.shape,labels_test.shape)

#input_shape = (146,100,50)


model = Sequential()
model.add(Conv1D(16,kernel_size=50,padding='same',activation="relu"))
model.add(BatchNormalization())
model.add(Conv1D(32,kernel_size=50,padding='same',activation="relu"))
model.add(BatchNormalization())
model.add(Flatten())
...
train_history = model.fit(data,labels,batch_size=16,epochs=20,verbose=1,validation_split=0.2)
