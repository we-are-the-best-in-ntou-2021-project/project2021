...
a = de.JasonDecoder(dataset_name=action_name,frame=50,dirname="dataset2",shift=10)
data, labels = a.decoding()
labels = np.array(labels)

data = data.reshape(data.shape[0],data.shape[1],data.shape[3]*data.shape[2])
data,data_test,labels,labels_test = train_test_split(data,labels,test_size = 0.25,random_state = 10)
